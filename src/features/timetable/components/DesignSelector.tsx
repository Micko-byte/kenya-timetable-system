  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 mt-4">
      <h3 className="text-sm font-display font-bold text-foreground mb-3 flex items-center gap-2">
        <Paintbrush className="w-4 h-4" /> Design Template
      </h3>
      <div className="grid grid-cols-5 gap-2">
        {themes.map(([key, t]) => (
          ...
          >
            <div className={`h-5 rounded-sm mb-1 ${t.headerBg}`} />
            <div className={`h-2.5 rounded-sm mb-0.5 ${t.dayBg}`} />
            <div className="h-1.5 rounded-sm bg-muted" />
            <span className="text-[8px] font-semibold text-foreground mt-0.5 block leading-tight">{t.name}</span>
          </button>
        ))}
      </div>
    </div>
