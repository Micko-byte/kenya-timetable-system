...
export default function SubjectManager({
  ...
  const allSubjects = Object.keys(SUBJECTS).filter(
    (s) => s !== 'BREAK' && s !== 'LUNCH' && s !== 'Games'
  );

  const filteredSubjects = filterLevel === 'all'
    ? allSubjects
    : getSubjectsByLevel(filterLevel).filter(s => s !== 'BREAK' && s !== 'LUNCH' && s !== 'Games');

  const handleAdd = () => {
    const trimmed = newSubject.trim();
    if (!trimmed) return;
    if (allSubjects.includes(trimmed) || customSubjects.includes(trimmed)) {
      toast({ title: 'Already exists', description: `"${trimmed}" is already in the list.`, variant: 'destructive' });
      return;
    }
    onAddSubject(trimmed);
    setNewSubject('');
    toast({ title: 'Added', description: `"${trimmed}" added to subjects.` });
  };
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 mt-4">
      ...
      {showJsonInput && (
        <div className="mb-3 p-3 bg-muted rounded-lg">
          <p className="text-xs text-muted-foreground mb-2">
            Paste JSON with format: {`{"subjects": ["Subject1", "Subject2"]}`} or{' '}
            {`{"grid": [[{"subject":"Math","teacher":"Mr. A"},...],...]}`}
          </p>
          <textarea
            value={jsonText}
          ...
          />
          <Button size="sm" onClick={handleJsonImport} className="mt-2 text-xs">
            Import
          </Button>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1 mb-2">
        ...
        {LEVEL_KEYS.map((key) => {
          const count = getSubjectsByLevel(key).filter(s => s !== 'BREAK' && s !== 'LUNCH' && s !== 'Games').length;
          return (
            <button
              key={key}
            ...
            >
              {EDUCATION_LEVELS[key].emoji} {EDUCATION_LEVELS[key].label} ({count})
            </button>
          );
        })}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto">
        {filteredSubjects.map((name) => {
          ...
          return (
            <span
              key={name}
              className={`px-2 py-0.5 text-[10px] rounded-full font-medium text-white flex items-center gap-1 ${colorClassMap[color] || 'bg-subject-default'}`}
            >
              {name}
            </span>
          );
